...
try:
    ...
    thop = None  # Force None to prevent segmentation faults on custom multi-output models
except ImportError:
    thop = None
...
class ValLossEarlyStopping:
    """
    Early stopping based on *minimum* validation loss (lower = better).

    Rationale (SwinYOLO V4):
      The default EarlyStopping watches max(fitness) which is dominated by mAP.
      On synthetic SLD data, mAP saturated at epoch 9 (val mAP=0.9947) locking the
      best-epoch counter — the training was then killed at epoch 59 (9+50) even though
      the model was still actively improving its val_loss after epoch 12's spike recovery.

      Monitoring val_loss is a more stable signal:
        - Not saturated by easy datasets
        - More sensitive to generalization quality per epoch
        - Recommended by mentor: "Consider val_loss or val_f1score for early stopping"

    Usage in train.py:
        val_loss_stopper = ValLossEarlyStopping(patience=opt.patience)
        val_loss = results[4] + results[5] + results[6]  # box + obj + cls
        stop = val_loss_stopper(epoch=epoch, val_loss=val_loss) or stop
    """

    def __init__(self, patience=30):
        """Initialize val-loss early stopping with given patience (epochs without val_loss improvement)."""
    ...
    def __call__(self, epoch, val_loss):
        """
        Check whether val_loss has improved. Returns True if training should stop.

        Args:
            epoch (int): Current epoch index.
            val_loss (float): Sum of val box_loss + obj_loss + cls_loss for this epoch.
        Returns:
            bool: True if patience exceeded → stop training.
        """
        ...
        if stop:
            LOGGER.info(
                f"[ValLoss-ES] Stopping: val_loss has not improved for {self.patience} epochs. "
                f"Best val_loss={self.best_val_loss:.6f} at epoch {self.best_epoch}.\n"
                f"To adjust patience use `--patience N` or disable with `--patience 0`."
            )
        return stop
        ...
        for k, v in self.ema.state_dict().items():
            if v.dtype.is_floating_point:  # true for FP16 and FP32
                if v.shape != msd[k].shape:
                    print(f"EMA Shape Mismatch: key='{k}' EMA_shape={v.shape} Model_shape={msd[k].shape}")
                v *= d
                v += (1 - d) * msd[k].detach()
